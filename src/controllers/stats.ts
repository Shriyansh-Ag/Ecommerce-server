...
import { myOrders } from "./order.js";
import { disconnect } from "process";
import { getCategories } from "./product.js";
...
export const getDashboardStats = TryCatch(async(req,res,next) => {
    ...
    if(myCache.has("admin-stats")) stats= JSON.parse(myCache.get("admin-stats") as string)
        ...
        const [thisMonthOrders,thisMonthProducts,thisMonthUsers,
            lastMonthOrders,
            lastMonthProducts,
            lastMonthUsers,
            allOrders,
...
export const getBarChart = TryCatch(async(req,res,next) => {
    ...
    if(myCache.has(key)) charts = JSON.parse(myCache.get(key) as string)
        ...
        const [SixMonthProducts,
            SixMonthUsers,
            twelveMonthOrders] =  await Promise.all([
